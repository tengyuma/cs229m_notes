...
\todo{Add the Question part in the notes.}
...
The design principle for contrastive learning is to find $\theta$ such that $\phi_{\theta}(x)$, $\phi_{\theta}(x^+)$ are close, while $\phi_{{\theta}}(x), \phi_{{\theta}}(x')$ are far away. \todo{Ask tengyu, what's the past works?}
...
\end{align}
\todo{ask tengyu, what's the details?}
