...
\begin{remark}
	\tnote{maybe abstractify this into a lemma?}
...
\begin{remark}
	\tnote{maybe let's abstactify this into a lemma or something like that so that we can quote later}
...
\tnote{`` Why does pretraining on unlabeled data with an unsupervised (self-supervised) loss help a wide range of downstream prediction tasks? three more concrete questions:
	\\pretraining helps label efficiency of downstream tasks\\
	pretraining can give “universal” representations\\
	pretraining provides robustness to distribution shift
'}

\subsec{Analysis of contrastive learning}\label{section:analysis_contrastive_learning}
Let $\bar{X}$ be the set of all natural images in the image domain, $\bar{P}_{\bar{X}}$ be the distribution over $\bar{X}$. Contrastive learning learns $\phi_{\theta}$ such that representations of augmentations of the same image are close, whereas augmentations of random images are far away. \todo{add figure}
...
The design principle for contrastive learning is to find $\theta$ such that $\phi_{\theta}(x)$, $\phi_{\theta}(x^+)$ are close, while $\phi_{{\theta}}(x), \phi_{{\theta}}(x')$ are far away. \todo{Ask tengyu, what's the past works?}\tnote{sorry I meant empirical works; just need to cite them I guess}
...
\end{align}
\todo{ask tengyu, what's the details?}\tnote{interpret why this encourage the principle (pushing positive pairs closer,etc)that we outlined before}
