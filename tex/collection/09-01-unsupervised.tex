...
The third moment is $\Exp[x \otimes x \otimes x] \in \mathbb{R}^{d \times d \times d}$. We can average over each of the Gaussians in the mixture to express this quantity in terms of the third moment of each Gaussian. Namely,
\begin{align}
	\Exp[x \otimes x \otimes x] = \frac{1}{k} \sum_{i=1}^k \Exp[x \otimes x \otimes x \mid i]
\end{align}

Then, we apply the subsequent lemma, which states the third moment for a multivariate Gaussian, to determine the third moment of the mixture.
...
\end{lemma}
Note that $\Exp[z] = v$, which means that we can compute $v \otimes v \otimes v$ from linear combination of $\Exp[z \otimes z \otimes z]$  and $\Exp[z]$.

\begin{proof}
We compute the third moment element-wise. Note that
\begin{align}
...
We can now apply Lemma~\ref{lec19:lma:gaussian_third_moment} to compute the third moment of the mixture. In particular,
\begin{align}
	\Exp[x \otimes x \otimes x] & =  \frac{1}{k} \sum_{i=1}^k \Exp[x \otimes x \otimes x \mid i] \\
...
The derivations above tell us how we might compute $\frac{1}{k} \sum_{i=1}^k \mu_i \otimes \mu_i \otimes \mu_i$ from $\Exp[x \otimes x \otimes x]$ and $\Exp[x]$. For notational convenience, let
\begin{equation}
    a^{\otimes 3} \defeq a \otimes a \otimes a.
\end{equation}
In the sequel, we describe how one might compute $\{\mu_i\}_{i = 1}^k$ from $\frac{1}{k} \sum_{i = 1}^k \mu_i^{\otimes 3}$. 

\paragraph{Tensor decomposition}
Recovering the Gaussian means $\{\mu_i\}_{i = 1}^k$ from the third moment of the mixture is a special case of the general tensor decomposition problem. This problem can be set up as follows. Assume that $a_1, \cdots a_k \in \bbR^d$ are unknown. Then, given $\sum_{i=1}^k a_i^{\otimes 3}$, our goal is to reconstruct the $a_i$ vectors. 
...
\end{equation} 
A crucial difference between tensors and matrices is that the former do not have the typical rotational invariance. In particular, there is no analogue to the standard matrix result that 
\begin{equation}
    AA^\top = A RR^\top A^\top.
\end{equation}
...
\begin{enumerate}
\setcounter{enumi}{-1}
\item  In the most general case, recovering the $a_i$'s from $T$ is computationally hard. Any procedure will either fail to find a unique $a_i$ or it fails to find them \emph{efficiently}. 
...
\end{equation}
We can heuristically think of $a_i$ as eigenvectors of $T$ and we can find $a_i$ in poly-time.
\item In the independent case, i.e. $a_1,\dots,a_k$ are linearly independent. Jennrich's algorithm can be used to find the $\{a_i\}_{i = 1}^k$.
\end{enumerate} 
Results 1 and 2 in the above list both involve the so-called ``under-complete'' case ($k \leq d$), e.g., when the number of Gaussians in the mixture is smaller than the dimension of the data. Next, we describe certain overcomplete cases for which recovery is possible.
\begin{enumerate}
\setcounter{enumi}{2}
...
\item Excluding an algebraic set of measure $0$, we can use the FOOBI algorithm to recover $a_i$ from the fourth-order tensor $\sum_{i = 1}^k a_i^{\otimes 4}$ when $k \leq d^2$. \citet{ma2016poly} introduce a robust version of the FOOBI algorithm.
\item Assume $a_i$ are \emph{randomly} generated unit vectors. Then, for the 3rd order tensor, $k$ can be large as $d^{1.5}$ \cite{ma2016poly, schrammsteurer17}. 
\end{enumerate}

In summary, the moment method is a recipe in which we first compute high order moments (i.e. tensors), assume that these estimates are noiseless, and decompose these tensors to recover the latent variables. Though we do not discuss these results here, there is an extensive literature analyzing the robustness of the moment method to error in the moment estimates. Though we only explicitly analyze the mixture of Gaussians model here, latent variable models amenable to analysis by the moment method include ICA, Hidden Markov Models, topic models, etc.

\sec{Spectral Clustering}
Introduced by \citet{shi2000normalized} and \citet{ng2002spectral}, spectral clustering learns a model for the data points using a \emph{pairwise} notion of similarity. Formally, assume that we are given $n$ data points $x\sp{1}, \dots, x\sp{n}$ as well as a similarity matrix $G \in \bbR^{n\times n}$ where 
\begin{equation}
...
For example, $x\sp{i}$ could denote images for which $\rho (x\sp{i}, x\sp{j})$ measures the semantic similarity. Alternatively, $x\sp{i}$ might be users of social network where $\rho (x\sp{i}, x\sp{j}) = 1$ if $x\sp{i}$ and $x\sp{j}$ are friends. 

Our goal is to cluster the data points by viewing $G$ as a graph. For instance, in the social network example, we can naturally view $G$ as an \emph{unweighted} graph, where $G_{ij} \in \{0, 1\}$ defines the edges. Then, the clustering problem is to partition the graph into components that are separate from each other. As we will see repeatedly in the sequel, the eigendecomposition of $G$ is closely related to this graph paritioning problem.
...
In some trivial cases, it is not necessary to eigendecompose $G$ to recover the two hidden communities. Suppose, for instance, that $p = 0.5$ and $q = 0$. Then, the graph should contain two connected components that correspond to $S$ and $\bar{S}$.

As a warm-up to motivate our approach, we eigendecompose $\bar{G} = \Exp[G]$. Observe that
\begin{align}
    \bar{G}_{ij} = \begin{cases}
        p &\text{if $i,j$ from same community} \\
        q &\text{o.w.} \end{cases}.
\end{align}
It is then easy to verify that $\bar{G}$ is a matrix of rank $2$:
\begin{align}
    \bar{G} = \left[
        \begin{array}{c|c}
        p \cdots p & q \cdots q \\
...
\end{align}
More generally, $\vec{1}$ is the top eigenvector for any matrix with fixed row sum or for any graph with uniform degree. 
...
\begin{align}
    G' = \left[
        ...
        \right]
\end{align}
for $r = p - q$. Note that $G'$ is then block diagonal with two blocks of size $|S|$ and $|\bar{S}|$, respectively. Then, 
\begin{align}	
	\bar{G} &= \vec{1} \cdot \vec{1}^\top q + G'. \label{lec19:eqn:barG}
\end{align}
Thus,
\begin{align}
 G' \cdot u &= \left[
...
\end{align}
Furthermore, the rows of the matrix formed by the three eigenvectors given by \eqref{lec19:eqn:eigenmatrix} will also provide the cluster IDs of the vertices in $G$. Note also that permutations of $A$ will result in equivalent permutations in the coordinates of each of the three eigenvectors.

While there are no negative values in the eigenvectors given in \eqref{lec19:eqn:eigenmatrix}, note that any linear combination of eigenvectors is also an eigenvector, so recovering eigenvectors that look like \eqref{lec19:eqn:second_eig_G} is straightforward. Taking linear combinations of the eigenvectors defined above shows that there is an alternative eigenbasis that includes the all-ones vector, $\vec{1}$. But for this choice of $A$, the all-ones vector is not the unique top eigenvector. For that condition to hold, we require background noise in $\bar{G}$.
