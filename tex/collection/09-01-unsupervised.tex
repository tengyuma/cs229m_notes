...
\end{equation}
be the normalized adjacency matrix. This is equivalent to normalizing each element $G_{ij}$ \tnote{typo?}of the adjacency matrix by $\frac{1}{\sqrt{d_i d_j}}$.\tnote{gives an explicit formula for the entry of A bar}
...
We will refer the readers to~\cite{?} for the proof of the theorem. Here below we give a few basic lemmas that help build up intuitions on why eigendecompositions relate to graph decomposition.
...
Why does pretraining on unlabeled data with an unsupervised (self-supervised) loss help a wide range of downstream prediction tasks? There are many open questions to be answered in this field. For instance,  we may ask: (1) how does pretraining help label efficiency of downstream tasks\tnote{the be verb should be after the noun unless this is a standalone question}, (2) why pretraining can give ``universal'' representations, and (3) why does pretraining provide robustness to distribution shift. 
...
\item{\textbf{Key step:} (infinite data case) We will prove a theorem (Theorem ?? \tnote{add pointer})that shows if $L(\hat{\theta})\le2\epsilon$, then there exists $w$ such that $L_{\text{task}}(\theta, w)\le \delta$, where $\delta$ is a function of $\epsilon$ and data distribution $\bar{P}$.}
...
Standard matrix decomposition results tell us that the minimizer of $\norm{\bar{A}-FF^\top}_F^2 $ satisfies $F = U \cdot \text{diag}(\gamma_i^{\frac{1}{2}})$, where $\gamma_i$'s are the eigenvalues of $\bar{A}$ and $U\in\bbR^{N\times k}$ contains the top $k$ eigenvectors of $\bar{A}$ as its columns. Suppose we use $u_1, \cdots, u_N$ \tnote{I think we need to change the notation here because $v_i$ is used to denote the columns already; use $v_i$?} to represent the rows of $U$, i.e., 
	\begin{equation}
...
One interesting thing is that $\phi(x_i)$ has the same separability as $u_i$. This is because for any vector $v\in\bbR^k$\tnote{change $u_i$ to $v_i$ and change $v$ to something else e.g., $\theta$}, we have $\id{v^\top u_i>0} = \id{v^\top \text{diag}(\gamma_i^{-\frac{1}{2}}) f(x) >0}$, suggesting linear head $\text{diag}(\gamma_i^{-\frac{1}{2}}) v$ applied on feature $f(x_i)$ would achieve the same classification accuracy as $v$ applied on $u_i$. Thus, it suffices to analyze $u_i$'s downstream accuracy under linear head, which is exactly the spectral clustering literature has studied\tnote{this doesn't sound to be true? the literature didn't study linear probe?}.

When does spectral clustering produce good features? Recall that we've shown spectral clustering is good at graph partitioning in stochastic block model or the worst-case graph\tnote{this sentence seems to come out of nowhere; more transition}. For simplicity, let's consider a regular graph where $w(x) = \sum_{x'\in V} w(x, x') = d$. \tnote{could we change $d$ to $\kappa$--- it coincides the dimension }

\tnote{maybe say what the lemma is about in words before stating the lemma}
...
Before proving the above lemma, we first introduce the following higher-order Cheeger inequality\tnote{say briefly what its about}. 
\begin{lemma}[Proposition 1.2  in \cite{louis2014approximation}]\label{lemma:higher_order_cheeger}
	...
	\end{align*}
	\tnote{can we have the other side of the inequality stated here as well? }
...
\begin{proof}[Proof of Lemma~\ref{lemma:ssl1}]
	By results in Section~\ref{subsec:clustering_worst_graph} \tnote{which equation}we know that 
\begin{align}
