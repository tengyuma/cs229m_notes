...
\begin{remark}
\tnote{I like the idea of moving the intuitions before the theorem, though let's reserve remark environment for more or less modularized discussions. So I think we can just directly have the texts here without the remark environment}
...
\begin{proof} 
    ...
    \tnote{I don't know why change to $\pi_i(v)$. Then should be $u_i$}
    ...
    \begin{figure}[h!]
        ...
        \end{center}
        \caption{We observe that $\log N(\epsilon, \cF, L_2[P_n])$ is monotone decreasing in $\epsilon$. As a result, the right Riemann sum in \eqref{lec9:eqn:dudley_rriemann} will be upper bounded by the integral.\tnote{improve the figure according to my email}}
