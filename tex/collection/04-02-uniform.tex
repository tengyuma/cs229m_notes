...
This complexity measure will depend on the distribution $p$ over $\cX \times \cY$ (the input and output spaces), and hence takes into account how easy it is to learn $p$. If $p$ is easy to learn, then this complexity measure will be small even if the hypothesis space is big. \tnoteimp{distribution should be $P$ instead of $p$; please check globally}
...
\end{equation}
The expectation is over the randomness in the training data ($\cX \times \cY$). (Note: We cannot just swap the order of $\bbE$ and $\sup$! \tnoteimp{more formal})
...
\end{align}
\tnoteimp{should use the given official macro $\Exp$, which also put the symbols below the E. remove completely the new macro bbE} 
...
\begin{remark}
For our applications \tnoteimp{to clarify; it seems to mean an empirical application} we will take $\cZ = \cX \times \cY$. However, Definition \ref{lec5:dfn:rc} holds for abstract input spaces $\cZ$ as well.
\end{remark}
...
\begin{theorem} \label{lec5:thm:thm1}
    ...
    \end{align}
    \tnoteimp{typo: $f(x)$ to $f(z)$}
\end{theorem}

\begin{remark}
We can think of $\frac{1}{n} \sum^n_{i=1} f(z_i)$ as an empirical average and $\bbE_{z\sim P} f(x)$     \tnoteimp{typo: $f(x)$ to $f(z)$} as a population average.
\end{remark}
...
\begin{align}
    ...
    &=\frac{1}{2} \bbE_{(x^{(i)}, y^{(i)}), \sigma_i} \l[\sup_{h \in \cH} \frac{1}{n}\sum^n_{i=1} h(x^{(i)})\sigma_i \r] &(-y_i \sigma_i \stackrel{d}{=} \sigma_i) \tnoteimp{typo} \\
    &= \frac{1}{2}R_n(\cH). &(\text{by definition})
\end{align}
\tnoteimp{what's below the expectation should be $\{(x^{(i)}, y^{(i)})\}$ }
...
\end{align}
since the outer $\sup$ \tnoteimp{which outer sup?}becomes vacuous.
