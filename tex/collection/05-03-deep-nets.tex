...
Note, however, that neither of these approaches produce bounds that depend upon the data. By contrast, in the sequel, we will derive a new \textit{data-dependent} generalization bound. These bounds state that with high probability over the choice of the empirical data and, forall all functions $f\in \cF$, \tnote{parenthesis around $x\sp{i}, y\sp{i}$--there are other occurrences, plz fix them as well}
\begin{align}
...
\end{align}
\tnote{I guess we are waiting for the figures from the scribe notes takers. Just a reminder to have a simple illustrative figure here} 
\tnote{I changed $g$ to $h$ here to avoid conflicting notations. Might need to change the notation in the figure as well.}
...
\begin{proof}
The high-level idea of our proof is to replace $\cF$ with $\cG$ before repeating a standard margin theory argument \tnote{provide a reference to previous section}.

Let $\ell_\gamma$ be the ramp loss, which is 1 for negative values, 0 for values greater than $\gamma$, and a linear interpolation between 1 and 0 for values between 0 and $\gamma$\tnote{provide a reference to previous section}. 
...
\end{align}
such that $f(x, \delta)y \leq 0$ \tnote{perhaps it's slightly better to put this constraint in the displayed equation} (i.e. the perturbation yields incorrect predictions).
...
\begin{corollary}\tnote{slightly more formal statement [e.g., assuming .. by taking epsilon .. in theorem .. , we have ]}
...
\tnote{I assume $cF_{r-1}'$ is a typo?}
 $|m_f(x, y) - m_{f'}(x, y)| \leq \sqrt{\sum_{i=1}^r \max_{\norm{x}_2 \leq 1} \norm{f_i(x) - f_i'(x)}_2}$. \tnote{I might worths putting this into a displayed equation}
...
Having established the validity of our cover, we now return to our claim of 1-Lipschitz-ness in step 1 of this proof.\tnote{maybe more formally refer to the equation to prove with a reference of the equation number}
 By symmetry, it is sufficient to prove an upper bound for $m_{f'}(x, y) - m_f(x, y)$.
...
\tnote{the footnote about Minkowski seems to be not working in the math mode. I think there are workaround online (I don't remember exactly)}
\tnote{the step for the Minkowski inequality is missing a square}
%\tnote{it's good to say the third line follows from 1-homogeneity}
\begin{align}
