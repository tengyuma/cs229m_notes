...
\begin{enumerate}
    ...
    \item It suffices to optimize in $B(\theta^0)$: There exists an approximate global minimum $\hat{\theta} \in B(\theta^0)$, so $\hat{L}(g_{\hat{\theta}}) \approx 0$. Intuitively, this is the lowest possible loss, which implies we are close to the global minimum and that $\hat{L}(f_{\hat{\theta}}) \approx 0$ as well.
