This chapter provides various generalization bounds for neural network models, and can be viewed as an extension of Chapter~\ref{chap:gen-bounds} with more involved techniques and stronger conclusions. 
...
%In Chapter~\ref{chap:dl-overview}, we will give a high-level overview of current landscape of deep learning theory, especially focusing on the difference between the theories for deep learning and classical machine learning. We put this chapter as the first chapter in this part due to its technical proximity to the previous chapter. Readers should feel free to skip this chapter and jump to Chapter~\ref{chap:dl-overview} for an overarching overview.   


\sec{Two-layer neural networks}

In this section, we will use the same language and framework as Chapter~\ref{chap:gen-bounds} and compute a bound for the Rademacher complexity of two-layer neural networks. We start by defining formally a two-layer neural network. Recall that the input is in $\R^d$ and the output is a real number, and he training datasets is $\{ (x\sp{i}, y\sp{i}) \}_{i=1}^n$ with $x\sp{i}\in\R^d$ and $y\sp{i}\in\R$.
%The following notations for neural networks will be used throughout this section

\begin{itemize}
    \item Let $\theta = (w, U)$ be the parameters of the two-layer neural net, where $U \in \R^{m \times d}$ is the first layer weights and $w \in \R^m$ is the second layer weights. Here $m$ denotes the number of hidden units. We use $u_i\in\R^d$ to denote the $i$-th row of $U$, and it's written as a column vector.
    \item Let $\phi(z) = \max(z, 0)$ be the ReLU activation function applied element-wise.
    \item Given input the $x$, the model with parameter $\theta$ outputs $f_\theta(x) = \inprod{w,\phi(Ux)} = w^\top \phi(Ux)\in \R$. 
\end{itemize}
We start with a  weak bound on the Rademacher complexity which introduces the technical tools that we need to derive tighter bounds subsequently.

\begin{theorem}\label{lec7:thm:thm_3}
    For some constants $B_w > 0$ and $B_u > 0$, let the hypothesis class $\cH$ be neural networks with bounded $\ell_2$ norms in the following sense
    \begin{equation}
        \mathcal{H} = \cbr{ f_\theta \mid \Norm{w}_2 \leq B_w,\ \Norm{u_i}_2 \leq B_u,\ \forall i \in \{1,2,\dots,m\} }\perm\label{lec7:eqn:thm_3}
    \end{equation}
     We also assume that the norm of the data are bounded in the sense of $\Exp[\Norm{x}_2^2]\leq C^2$. Then, 
    \begin{align}
        R_n(\mathcal{H}) \le 2 B_w B_u C\sqrt{\frac{m}{n}}.
    \end{align}
\end{theorem}
This Theorem indicates that $B_wB_uC\sqrt{m}$ is a complexity measure of the neural net that can control the generalization error. As discussed in Section~\ref{chap:generalization:sec:margin}, this Rademacher complexity bound implies a generalization bound of the form $O\left(B_wB_uC\sqrt{\frac{m}{\gamma^2 n}}\right)$ where $\gamma$ is the margin for the classification problem. 

However, this bound is far from ideal as it increase as the number of neurons~$m$ increases. 
Empirical results suggest  that the generalization error at least does \emph{not} increase monotonically with $m$ \citep{belkin2019,neyshabur2017exploring}. 
This means that the bound becomes loose for very large $m$, and it measures the complexity of the neural network in a suboptimal way. 
%As more neurons are added to the model, thereby giving it more expressive power, studies have shown that generalization is improved \cite{belkin2019}. 
%This contradicts the bound above, which states that more neurons leads to worse generalization. 

We also note that the theorem can be generalized straightforwardly to the setting where the $w$ and $U$ are jointly constrained in the sense that we set 
\begin{align}
\mathcal{H} = \cbr{ f_\theta \mid \Norm{w}_2\cdot \left(\max_i\Norm{u_i}_2\right) \leq B}\,,\label{eqn:13}
\end{align}
 and obtain the generalization bound $        R_n(\mathcal{H}) \le 2 B C\sqrt{\frac{m}{n}}.$ However, the $\sqrt{m}$ dependency still exists under this formulation of $\cH$. 

Nevertheless, we now prove this bound to build up some techniques, and will discuss its refinement in Section~\ref{sec:refined-bounds} and the broader picture in Chapter~\ref{chap:dl-overview}. 

\begin{proof}[Proof of Theorem~\ref{lec7:thm:thm_3}] We have a few simplifications starting from the definition of Rademacher complexity:
    \begin{align}
        R_S(\mathcal{H}) 
        ...
        \\&= \frac{B_w}{n}\Exp_\sigma\sbr{ \sup_{U : \Norm{u_j}_2 \leq B_u} \Norm{ \sum_{i=1}^n \sigma_i \phi(Ux\sp{i})}_2 }
            &&\text{(bc. $\textstyle\sup_{\Norm{w}_2\leq B}\inprod{w,v} = B\Norm{v}_2$)}
	\end{align}
 The steps above allow us to remove the presence of variable $w$ inside the supreme. Next, we change the $\ell_2$ norm to $\ell_\infty$ norm so that the matrix variable $U$ can be reduced to a vector variable. 
        \begin{align}
               R_S(\mathcal{H}) &  \leq \frac{B_w\sqrt{m}}{n}\Exp_\sigma\sbr{ \sup_{U : \Norm{u_j}_2 \leq B_u} \Norm{ \sum_{i=1}^n \sigma_i \phi(Ux\sp{i})}_\infty }
            &&\text{(bc. $\Norm{v}_2 \leq \sqrt{m}\Norm{v}_\infty$)}
        \\&= \frac{B_w\sqrt{m}}{n}\Exp_\sigma\sbr{ \sup_{U : \Norm{u_j}_2 \leq B_u} \max_{1\leq j\leq m} \abs{ \sum_{i=1}^n \sigma_i \phi(u_j^\top x\sp{i})} }  \label{eqn:14}
        ...
        \\&\leq \frac{2B_w\sqrt{m}}{n}\Exp_\sigma\sbr{ \sup_{\Norm{u}_2 \leq B_u} \sum_{i=1}^n \sigma_i \phi(u^\top x\sp{i}) }
            &&\text{(by Lemma \ref{lec8:lemma:absfortwo}, stated and proved later)} \label{lec7:eqn:nn-proof1}
        \\&\leq \frac{2B_w\sqrt{m}}{n}\Exp_\sigma\sbr{ \sup_{\Norm{u}_2 \leq B_u} \sum_{i=1}^n \sigma_i u^\top x\sp{i} }, \label{lec7:eqn:nn-proof2}
    \end{align}
    where the last inequality follows by applying the contraction lemma (Talagrand's lemma, Lemma~\ref{lec6:lem:talagrand_lemma}) and observing that the ReLU function is $1$-Lipschitz. Observe that the expectation in \eqref{lec7:eqn:nn-proof1} is the Rademacher complexity for the model family $\{ x \mapsto \phi(u^\top x) \mid \Norm{u}_2 \leq B_u \}$, to which we apply the contraction lemma. Here line~\ref{lec7:eqn:nn-proof1} removes the absolute value inside the supremum at the expense of paying a constant factor of 2. This is a standard tool in the literature of Rademacher complexity and we will defer its formal statement and proof at the end of this section. 
    
    We now observe that the expectation in \Cref{lec7:eqn:nn-proof2} is the Rademacher complexity of the family of linear models $\{x \mapsto \inprod{u,x} \mid \Norm{u}_2\leq B_u\}$. Thus, applying Theorem~\ref{lec7:thm:l1-thm} for linear models yields
    \begin{equation}
        R_S(\mathcal{H}) \leq \frac{2B_w\sqrt{m}}{n}B_u\sqrt{\sum_{i=1}^n \Norm{x\sp{i}}_2^2}\perm\label{eqn:12}
    \end{equation}
    
    Taking the expectation of both sides of \Cref{eqn:12}  and using similar steps to those in the proof of Theorem~\ref{lec7:thm:l1-thm} yields
    \begin{align}
        ...
        &= 2 B_w B_u C\sqrt{\frac{m}{n}}, \label{eqn:15}
    \end{align}
...
\subsec{Refined path-norm bounds}\label{sec:refined-bounds}
\newcommand{\boundsforcomp}{B}
Next, we look at a finer bound that results from defining a new complexity measure. 
A recurring theme in subsequent proofs will be looking for complexity measures that are more invariant to transformations of neural network parameters that preserve the functionality of the neural network. 

A technical  example is the following: given a neural net $\theta = (w, U)$, let $u_j$ be the row of $U$ with the smallest $\ell_2$ norm. Suppose we split the neuron $j$ into two neurons by replacing $w_j, u_j$ with $w_j'=w_j/\sqrt{2}$ and $w_j'' = w_j/\sqrt{2}$ and replacing $u_j$ with  $u_j'=u_j/\sqrt{2}$ and $u_j'' = u_j/\sqrt{2}$. We obtain a neural network  with exactly the same functionality. The $\ell_2$ norm of $w$  and the maximum row norm of $U$ remains the same, and thus the quantity $\Norm{w}_2\cdot \left(\max_i\Norm{u_i}_2\right)$ (in \Cref{eqn:13}) remains the same, but the number of neuron increase by 1, which means the generalization bound increases. Such a discrepancy may not be a disaster---we should not always expect generalization bounds to be tight---, but it is certainly desirable to fix it. 


 %will be the functional invariance of two-layer neural networks under a class of rescaling transformations. 
The key ingredient of these transformation of neural network parameters that preserve its functionality is  the \textit{positive homogeneity} of the ReLU function, i.e., 
\begin{equation}
...
\end{equation}
In light of this, we devise a new complexity measure $C(\theta)$ that is also invariant under such transformations and use it to prove a better bound for the Rademacher complexity. This invariance is absent in the complexity measure used in the hypothesis class \eqref{lec7:eqn:thm_3} of Theorem \ref{lec7:thm:thm_3} or the hypothesis class \eqref{eqn:13} .

\begin{theorem}\label{lec8:thm:thm-improved-nn-rc}
$\operatorname{Let} C(\theta)=\sum_{j=1}^{m}\left|w_{j}\right|\left\|u_{j}\right\|_{2}$. For some constant $\boundsforcomp>0$, consider the hypothesis class
\begin{equation}
\mathcal{H}=\left\{f_{\theta} \mid C(\theta) \leq \boundsforcomp\right\}\perm \label{eqn:H}
\end{equation}
Assume that $\left\|x\sp{i}\right\|_{2} \leq C$ for all $i \in\{1, \ldots, n\}.$ Then, we have
\begin{equation}
R_{S}(\mathcal{H}) \leq \frac{2 \boundsforcomp C}{\sqrt{n}}\perm
\end{equation}
\end{theorem}
The complexity measure $C(\theta)$ is closely related to the path norm defined in~\citet{neyshabur2015path}. In this monograph, we refer to it as a form of path norm. 

In contrast with Theorem~\ref{lec7:thm:thm_3}, this bound does not \textit{explicitly} depend on the number of neurons $m$. Thus, it is possible to use more neurons and still maintain a tight bound as long as the value of the new complexity measure $C(\theta)$ does not increase as we increase the number of neurons. The bound of Theorem \ref{lec7:thm:thm_3}, on the other hand, explicitly grows with the total number of neurons, and thus a good generalization bound with a larger number of neurons is impossible by using Theorem~\ref{lec7:thm:thm_3}. In fact, Theorem~\ref{lec8:thm:thm-improved-nn-rc} is strictly stronger than Theorem~\ref{lec7:thm:thm_3} as elaborated below. Note that 
	\begin{align}
	...
	\end{align}
	Therefore, if we consider $\cH^1 = \{f_\theta \mid \sum |w_j|\|u_j\|_2\le B'\}$ and $\cH^2 = \{f_\theta \mid \|w\|_2 \cdot \sqrt{m} \cdot \max_{j}\|u_j\|_2 \le B'\}$, then either Theorem~\ref{lec8:thm:thm-improved-nn-rc} on $\cH^1$ or Theorem~\ref{lec7:thm:thm_3} on $\cH^2$ gives the same Rademacher complexity bound $O(B'/\sqrt{n})$, but $\cH^1 \supset \cH^2$. 
	
	Moreover, interestingly, Theorem~\ref{lec8:thm:thm-improved-nn-rc} give a \textit{stronger} conclusion as we have more neurons. Let $\cH_m$ be the hypothesis class $\left\{f_{\theta} \mid C(\theta) \leq \boundsforcomp\right\}$ where $\theta$ uses $m$ neurons (the dependency on $m$ was omitted in the context of Theorem~\ref{lec8:thm:thm-improved-nn-rc} because $m$ was considered as fixed there). We have 
	$\cH_m \subset \cH_{m+1}$ whereas the Rademacher complexity bound for $\cH_{m+1}$ is the \textit{same} as $\cH_m$. This means that the bound for $\cH_{m+1}$ implies the bound for $\cH_m$. 	
	%this is because the hypothesis class $\cH$ as defined in~\eqref{eqn:H} is bigger as $m$ increases. 
	Because of this property, Theorem~\ref{lec8:thm:thm-improved-nn-rc} can be  possibly used to obtain a generalization guarantees that decrease as $m$ increases, as discussed in Section~\ref{sec:gen-bounds:decreasing-in-m}. 
...
Due to the positive homogeneity of the ReLU function $\phi$, it will be useful to define the $\ell_2$-normalized weight vector $\bar{u}_j \defeq u_j / \norm{u_j}_2$ and rewrite $\phi\left(u_j^\top x\right) = \norm{u_j}_2 \cdot \phi(\bar{u}_j^\top x)$. Essentially we will merge the $\|u_j\|_2$ component into the second layer weight $w_j$, and treat $\bar{u}_j$ as the first layer weight vector. This allows us to capture the scaling invariance of the neural network in the proof. 


Starting form the definition of  empirical Rademacher complexity, we first peel off $w$ and the norm of $u_j$'s from the supremum---we compute the maximizers and remove the presence of these variables in the supremum. 
\allowdisplaybreaks
\al{
R_S(\cH) &= \frac{1}{n}\Exp_{\sigma}\left[ \sup_{\theta} \sum_{i=1}^n \sigma_i f_{\theta}\left(x\sp{i}\right) \right] \\
&= \frac{1}{n}\Exp_{\sigma}\left[ \sup_{\theta} \sum_{i=1}^n \sigma_i \left[\sum_{j=1}^m w_j \phi\left(u_j ^\top x\sp{i}\right) \right] \right] &&\text{(by definition of $f_\theta$)} \\
&=  \frac{1}{n}\Exp_{\sigma}\left[ \sup_{\theta} \sum_{i=1}^n \sigma_i \left[\sum_{j=1}^m w_j \norm{u_j}_2  \phi\left(\bar{u}_j ^\top x\sp{i}\right) \right] \right]  
    && \text{(by positive homogeneity of $\phi$)}\\
&= \frac{1}{n}\Exp_{\sigma}\left[ \sup_{\theta}  \sum_{j=1}^m w_j \norm{u_j}_2 \left[ \sum_{i=1}^n \sigma_i  \phi\left(\bar{u}_j ^\top x\sp{i}\right) \right] \right] \\ 
&\leq \frac{1}{n}\Exp_{\sigma}\left[ \sup_{\theta}  \sum_{j=1}^m |w_j| \norm{u_j}_2 \max_{k \in [n]}\left| \sum_{i=1}^n \sigma_i  \phi\left(\bar{u}_k ^\top x\sp{i}\right) \right| \right] && \l(\because \sum_j \alpha_j \beta_j \leq \sum_j |\alpha_j| \max_{k} |\beta_k|\r) \\ 
&\leq \frac{\boundsforcomp}{n} \Exp_{\sigma}\sbr{ \sup_{\theta = (w, U)} \max_{k \in [n]} \left| \sum_{i=1}^n \sigma_i  \phi\left(\bar{u}_k ^\top x\sp{i}\right) \right| } && \text{($\because C(\theta) \leq \boundsforcomp$)}  \label{eqn:16}
}
The rest of the proof is similar to \Cref{eqn:14} to \Cref{eqn:15} in the proof of \Cref{lec7:thm:thm_3}. The main difference is that work with $\bar{u}_j$'s now instead of $u_j$'s. 
\begin{align}
R_S(\cH) &\le  \frac{\boundsforcomp}{n} \Exp_{\sigma}\sbr{ \sup_{\bar{u}: \norm{\bar{u}}_2 = 1} \left| \sum_{i=1}^n \sigma_i  \phi\left(\bar{u} ^\top x\sp{i}\right) \right| } && \text{(by \Cref{eqn:16} and change of variables)}\\
%&\le \frac{\boundsforcomp}{n} \Exp_{\sigma}\sbr{ \sup_{\bar{u}: \norm{\bar{u}}_2 \le 1} \left| \sum_{i=1}^n \sigma_i  \phi\left(\bar{u} ^\top x\sp{i}\right) \right| } \\
&\le \frac{2\boundsforcomp}{n}  \Exp_{\sigma}\sbr{ \sup_{\bar{u}: \norm{\bar{u}}_2 \le 1} \sum_{i=1}^n \sigma_i  \phi\left(\bar{u} ^\top x\sp{i}\right) } && \text{(see Lemma \ref{lec8:lemma:absfortwo})} \\
&= 2\boundsforcomp R_S(\cH '),
\end{align}
where $\cH' = \l\{x \mapsto \phi(\bar{u}^\top x) :  \bar{u} \in \mathbb{R}^d, \norm{\bar{u}}_2 \leq 1 \r\}$. By Talagrand's lemma, since $\phi$ is $1$-Lipschitz, we have $R_S(\cH') \leq R_S(\cH'')$ where  $\cH'' = \l\{x \mapsto \bar{u}^\top x :  \bar{u} \in \mathbb{R}^d, \norm{\bar{u}}_2 \leq 1 \r\}$ is a linear hypothesis space. By \Cref{lec7:thm:l2-thm}, we have $R_S(\cH'') \leq \frac{C}{\sqrt{n}}$. Combining these facts concludes the proof.

\end{proof}

Finally, we state formally the Lemma \ref{lec8:lemma:absfortwo} that deal with the absolute value and prove it. 
\begin{lemma}\label{lec8:lemma:absfortwo}
Let $\sigma = (\sigma_1, ..., \sigma_n)$ and $f_{\theta}(x) = \l(f_{\theta}\l(x\sp{1}\r), ...,  f_{\theta}\l(x\sp{n} \r)\r)\in \R^n$. Suppose that for any $\sigma \in \{\pm 1\}^n$, $\sup_{\theta} \langle \sigma, f_{\theta}(x) \rangle \geq 0$. Then, 
\begin{equation}
...
\end{equation}
\end{lemma}
Notably, the lemma's assumption holds in the context of the proof of \Cref{lec7:thm:thm_3} and \Cref{lec8:thm:thm-improved-nn-rc}. For example, in the context of line~\eqref{lec7:eqn:nn-proof1}, we have 
\al{\sup_{u: \norm{u}_2 \leq B_u} 
	\sum_{i=1}^n \sigma_i \phi \l(u^\top x\sp i \r)  \geq   0\perm
} 
%This is because  one can take $\bar{u} = 0$ for any $\sigma = (\sigma_1, \dots, \sigma_n)$.


\begin{proof}[Proof of Lemma~\ref{lec8:lemma:absfortwo}]
...
\sec{Implications of the generalization bounds and discussions}
