...
\begin{proof}[Proof of Theorem~\ref{lec7:thm:thm_3}] We have a few simplifications starting from the definition of Rademacher complexity:
 ...
 The steps above allow us to remove the presence of variable $w$ inside the supreme. Next, we change the $\ell_2$ norm to $\ell_\infty$ norm so that the matrix variable $U$ can be reduced to a vector variable. 
    ...
    \end{align}
    where the last inequality follows by applying the contraction lemma (Talagrand's lemma, Lemma~\ref{lec6:lem:talagrand_lemma}) and observing that the ReLU function is $1$-Lipschitz. Observe that the expectation in \eqref{lec7:eqn:nn-proof1} is the Rademacher complexity for the model family $\{ x \mapsto \phi(u^\top x) \mid \Norm{u}_2 \leq B_u \}$, to which we apply the contraction lemma.
