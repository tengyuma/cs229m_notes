...
As an example, consider linear classification, where the model outputs $\ind{w^\top x \ge 0}$ as the prediction. The most direct approach is to define the family of hypothesis $\cH = \{x\mapsto \ind{w^\top x \ge 0}: w\in \R^d\}$, and the loss function $\ell: \{0, 1\}\times \{0, 1\}\rightarrow \R$ is defined to be $\ell(\hat{y}, y) =\ind{\hat{y} \neq y}$. This formulation is essentially the same as in Example~\ref{example:binary-classficiation-rc} except that the label set in that example is  $\{\pm 1\}$ for convenience. The first drawbacks of this formulation is that  the loss function and the model are both discontinuous and thus challenging to reason about. More importantly, the hypothesis class is scale-invariant---we have $\ind{w^\top x \ge 0} = \ind{\alpha w^\top x \ge 0}$ for any positive $\alpha$---, and as a result, the formulation does not allow us to consider finer details about the classifier, e.g., the norm of $w$ and the ``confidence'' of the prediction. Note that $w^\top x$ is the logit in logistic regression, where the model eventually outputs a probability distribution for the two classes. The confidence of the classifier on training examples is intuitively supposed to be a quantity that affects the generalization of the classifier. 

It turns out that an alternative formulation allows us to consider the norm and ``confidence'' of the classifier
